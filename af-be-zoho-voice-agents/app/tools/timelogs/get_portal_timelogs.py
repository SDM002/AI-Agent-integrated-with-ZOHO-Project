"""Timelog read operation to fetch bulk logs across all projects (portal level) using Zoho v3 APIs."""
import asyncio
import json
from datetime import datetime, timedelta
from typing import Optional
from langchain_core.tools import tool
from app.tools.helpers import normalize_date, ok, err, resolve_user_by_email, parse_date_range
from app.utils.request_context import get_user_id
from app.services.zoho.client_factory import build_zoho_client

@tool
async def get_portal_timelogs(
    date: Optional[str] = None,
    owner_email: Optional[str] = None,
) -> str:
    """
    Fetch all time logs across all projects (portal-wide) for a date or date range (uses Zoho v3 API supporting "this week", etc.).
    Args:
    ...
    async with await build_zoho_client(user_id) as client:
        ...
        async def fetch_module(module_type: str):
            ...
            return await client.get_v3("/timelogs", params=p)
